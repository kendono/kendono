...
function getFileFromEditor(editor) {
  return editor.editorComponent.file;
}
...
function isPluginsAware(app) {
  return "plugins" in app;
}
function getPlugin(application, pluginName) {
...
var LinksSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(this.containerEl).setName("Capitalize link file name").setDesc("When selected then `text` creates link `[[Text|text]]`, otherwise `[[text|text]]`.").setDisabled(!this.plugin.settings.copyDisplayText).addToggle((component) => {
...
// src/TemplaterIntegration.ts
function getTemplaterPlugin(application) {
  const plugin = getPlugin(application, "templater-obsidian");
  return plugin ? new TemplaterWrapper(plugin) : void 0;
}
var TemplaterWrapper = class {
  constructor(templaterPlugin) {
...
var LinkInfo = class {
  constructor(linkStart, file, editor, makeAlias, cacheLink, linkText) {
...
var LinkWithAliasPlugin = class extends import_obsidian2.Plugin {
  ...
  createLinkFromSelection(file, editor, position, options) {
    ...
    if (!newCacheLink)
      throw new Error("cannot find newly create link");
    ...
    lastLink.register(
      ...
      this.editorCursorListener.fireOnCursorChange(editor, (cursorPosition) => {
        var _a;
        if (!cursorPosition) {
          this.addMissingAlias(lastLink.cacheLink, (_a = lastLink.file) == null ? void 0 : _a.path);
          return false;
        }
